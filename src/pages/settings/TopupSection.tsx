...
interface TopupSectionProps {
  ...
  showOnlyAmounts?: boolean;
  showOnlyStripe?: boolean;
}
...
export const TopupSection = React.memo(({
  ...
  onConnectStripe,
  isConnecting,
  showOnlyAmounts = false,
  showOnlyStripe = false
}: TopupSectionProps) => {
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Wallet className="w-5 h-5" /> {showOnlyStripe ? 'Auszahlungskonto (Stripe)' : 'Guthaben & Aufladung'}</CardTitle>
          <CardDescription>
            {showOnlyStripe 
              ? 'Verknüpfe dein Bankkonto für Online-Aufladungen.' 
              : 'Konfiguriere, wie deine Kunden ihr Guthaben aufladen können.'}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          {!showOnlyAmounts && (
            <div className={`pt-4 ${!showOnlyStripe ? 'border-t border-border/50' : ''}`}>
